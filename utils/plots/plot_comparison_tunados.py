...
ours_psnr = ours_psnr_raw[
    (ours_psnr_raw["mask_type"] == "erp")
    & (ours_psnr_raw["wsmse_tag"] == 1)
    & (ours_psnr_raw["swhdc_tag"] == 1)
].copy()

# 3. Ours - Tunado para SSIM
ours_ssim_raw = pd.read_csv(PATH_OURS_SSIM)
ours_ssim = ours_ssim_raw[
    (ours_ssim_raw["mask_type"] == "erp")
    & (ours_ssim_raw["wsmse_tag"] == 1)
    & (ours_ssim_raw["swhdc_tag"] == 1)
].copy()
...
make_panel(
    ...
    "eval_psnr",
    "WS-PSNR (dB)",
    "Rate-Distortion (WS-PSNR)",
)
plt.tight_layout(pad=1.5)
out_psnr = "experiments/01/rd_comparison_psnr.png"
plt.savefig(out_psnr, dpi=150, bbox_inches="tight", facecolor=BG_COLOR)
plt.close()
...
make_panel(
    ...
    "ws_ssim",
    "WS-SSIM",
    "Rate-Distortion (WS-SSIM)",
)
plt.tight_layout(pad=1.5)
out_ssim = "experiments/01/rd_comparison_ssim.png"
plt.savefig(out_ssim, dpi=150, bbox_inches="tight", facecolor=BG_COLOR)
plt.close()
...
plt.tight_layout(pad=1.5)
out_combined = "experiments/01/rd_comparison_combined.png"
plt.savefig(out_combined, dpi=150, bbox_inches="tight", facecolor=BG_COLOR)
